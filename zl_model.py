...
def Hw9_model(
    MATERIAL, CAPACITY, CUSTOMER, TIME,
    ...
    """Multiple-level multiple-product resource allocation model with substitution,
    procurement and multiple demands (store, online, etc.), and different sources
    of labor modeled as operations.
    """

    # Solver (use SCIP instead of Gurobi)
    solver = pywraplp.Solver.CreateSolver('SCIP')
    if solver is None:
        raise RuntimeError("SCIP solver is not available in this OR-Tools build.")
    ...
    stoc_pen = defaultdict(lambda: 0.5 * alpha, stoc_pen)
    make_pen = defaultdict(lambda: alpha, make_pen)

    # Add demand as supply for demand-resources
    for c in CUSTOMER:
        for r in MATERIAL:
            dem_vec = demand.get((r, c), [0] * len(TIME))
            if any(dem_vec):
                demand_res = f"D_{c}_{r}"
                for t in TIME:
                    t_index = TIME.index(t)
                    period_demand = dem_vec[t_index]
                    if period_demand > 0:
                        supply[(demand_res, t)] = period_demand

    # Map resource-level min buys to operation-level min buys
    ...
    for t in TIME:
        for op in OPERATIONS:
            if op in {'Op_Make_PickupTime', 'Op_Make_DeliverTime'} or op.startswith('Op_Trip_'):
                # binary schedule ops
    ...
    # Scrap penalty (for non-products)
    for r in MATERIAL.difference(PROD):
        if r == 'Money':
            continue
        for t in TIME:
            objective_terms.append(-scrap_pen[r] * Scrap[t, r])
    ...
    for r in MATERIAL:
        if r == 'Money':
            continue
        for t in TIME:
            objective_terms.append(-stoc_pen[r] * Stock[t, r])

    # Make / substitution penalties
    for t in TIME:
        for p in PROD:
            # base make op
            op_name = f"Make_{p}"
            if op_name in OPERATIONS and (t, op_name) in z:
                objective_terms.append(-make_pen[p] * z[t, op_name])

            # substitution make ops
            for (p_sub, r_sub, q_sub) in SUB_TRIPLES:
                if p_sub == p:
                    ...
                    if op_sub_name in OPERATIONS and (t, op_sub_name) in z:
                        objective_terms.append(-sub_pen[r_sub] * z[t, op_sub_name])

    # Constraints: resource balance
    for t in TIME:
        for r in RESOURCE:
            # produced in this period from earlier ops with offset
            sources_from_ops = []
            for op in OPERATIONS:
                if (op, r) in BOP:
                    off = offset_param.get((op, r), 0)
                    prev_t = prev(TIME, t, off)
            ...
            lhs = (
                (Stock[t, r] if r in MATERIAL else 0) +
                (Scrap[t, r] if r in RESOURCE.difference(PROD) else 0) +
                solver.Sum(
                    z[t, op] * usage_param.get((op, r), 0)
                    for op in OPERATIONS
                    if (t, op) in z and (op, r) in BOR
                )
            )

            rhs = (
                supply.get((r, t), 0) +
                (Stock[prev(TIME, t, 1), r] if (r in MATERIAL and TIME.index(t) > 0) else 0) +
                solver.Sum(sources_from_ops)
            )

            solver.Add(lhs == rhs, name=f'R_Balance[{t},{r}]')

    # Min-buy operation constraints
    for t in TIME:
        for op in HAS_MIN_OPS:
            if (t, op) in z and (t, op) in BinOp:
                solver.Add(BigM * BinOp[t, op] >= z[t, op],
                           name=f'ForceBinOp[{t},{op}]')
                solver.Add(z[t, op] >= BinOp[t, op] * min_buy_ops[op],
                           name=f'ForceMinOp[{t},{op}]')

    # "Either/Or" schedule constraint for Pickup vs Deliver on non-Friday
        ...
        for t in TIME:
            if t != friday:
                if (t, op_pickup) in z and (t, op_deliver) in z:
                    solver.Add(
                        z[t, op_pickup] + z[t, op_deliver] <= 1,
                        name=f'Doug_EitherOr[{t}]'
                    )

    # Limit total scrap for non-demand materials
    for r in MATERIAL.difference(PROD):
        if r == 'Money' or r.startswith('D_'):
            continue
        solver.Add(
            solver.Sum(Scrap[t, r] for t in TIME) <= supply.get((r, TIME[0]), 0),
            name=f'LimitScrap[{r}]'
        )

    # Terminal Money from revenue that arrives after horizon
    ...
    # Discounting of prices in objective (Ship ops)
    discount_terms = []
    for t in TIME:
        t_idx = TIME.index(t)
    ...
    # Solve
    solver.set_time_limit(150000)  # milliseconds
    gap = 0.01
    ...
    if status == pywraplp.Solver.OPTIMAL or status == pywraplp.Solver.FEASIBLE:
        ...
        # Pretty print operation variables
